...
public final class DetectionScanReportBuilder {
	...
	public static ScanReport build(
		...
		chatLines.add("detector: scan complete (targets=" + scannedTargets + ", matches=" + totalMatches + ")");
		if (details.isEmpty()) {
			chatLines.add("detector: no tracked entities or blocks found nearby.");
		} else {
			chatLines.addAll(details);
		}
		if (truncatedMatches > 0) {
			chatLines.add("detector: output truncated, " + truncatedMatches + " additional matches not shown.");
		}
		if (invalidTargets > 0) {
			chatLines.add("detector: skipped " + invalidTargets + " invalid target id(s); check /detector list.");
		}
	...
	private static String formatHit(DetectionScanHit hit) {
		String base = "detector: " + hit.kind().name().toLowerCase(Locale.ROOT)
			+ " " + hit.id()
