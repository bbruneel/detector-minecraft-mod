...
import org.bruneel.detector.client.detect.DetectionScanService;
import org.bruneel.detector.client.detect.DetectionTarget;
...
public final class DetectorClientCommands {
	...
	public static void register(
		...
		ClientCommandRegistrationCallback.EVENT.register((dispatcher, registryAccess) -> {
			...
			root.then(ClientCommandManager.literal("scan").executes(ctx -> {
				var source = ctx.getSource();
				...
				DetectorMod.LOGGER.info("Detect scan requested enabledTargets={}", enabledTargets.size());
			...
			root.then(ClientCommandManager.literal("precious_ores")
			...
			root.then(ClientCommandManager.literal("essential_ores")
				.executes(ctx -> applyPresetCommand(
					ctx.getSource(),
					registry,
					configStore,
			...
			root.then(ClientCommandManager.literal("all_ores")
				.executes(ctx -> applyPresetCommand(
					ctx.getSource(),
					registry,
					configStore,
			...
			root.then(ClientCommandManager.literal("highlightOnMatch")
				.then(ClientCommandManager.argument("value", boolArg()).executes(ctx -> {
					...
					ctx.getSource().sendFeedback(text("detector: highlightOnMatch=" + value));
					DetectorMod.LOGGER.info("Command detect highlightOnMatch updated value={}", value);
					return 1;
				})));

			root.then(kindLiteral(DetectionKind.ENTITY)
				.then(idArgWithSuggestions(DetectionKind.ENTITY)
					.then(ClientCommandManager.argument("enabled", boolArg()).executes(ctx -> {
						...
						ctx.getSource().sendFeedback(text(
							"detector: " + kind.name().toLowerCase() + " " + id + " enabled=" + enabled
						));
						DetectorMod.LOGGER.info("Command detect update kind={}, id={}, enabled={}", kind, id, enabled);
						return 1;
					}))));

			root.then(kindLiteral(DetectionKind.BLOCK)
					...
					.then(idArgWithSuggestions(DetectionKind.ENTITY)
						.then(ClientCommandManager.argument("value", doubleArg(1.0, 64.0))
							.executes(ctx -> {
								...
								ctx.getSource().sendFeedback(text("detector: radius updated for " + target.id() + " -> " + value));
								DetectorMod.LOGGER.info("Command detect radius update kind={}, id={}, value={}", target.kind(), target.id(), value);
								return 1;
							}))))
			...
			root.then(ClientCommandManager.literal("interval")
				.then(kindLiteral(DetectionKind.ENTITY)
					.then(idArgWithSuggestions(DetectionKind.ENTITY)
						.then(ClientCommandManager.argument("value", intArg(1, 1200))
							.executes(ctx -> {
								...
								ctx.getSource().sendFeedback(text("detector: interval updated for " + target.id() + " -> " + value));
								DetectorMod.LOGGER.info("Command detect interval update kind={}, id={}, value={}", target.kind(), target.id(), value);
								return 1;
							}))))
				.then(kindLiteral(DetectionKind.BLOCK)
					.then(idArgWithSuggestions(DetectionKind.BLOCK)
			...
			root.then(ClientCommandManager.literal("cooldown")
				.then(kindLiteral(DetectionKind.ENTITY)
					.then(idArgWithSuggestions(DetectionKind.ENTITY)
						.then(ClientCommandManager.argument("value", intArg(0, 72000))
							.executes(ctx -> {
								...
								ctx.getSource().sendFeedback(text(
									"detector: legacy cooldown updated for " + target.id()
			...
			root.then(ClientCommandManager.literal("messageCooldown")
				.then(kindLiteral(DetectionKind.ENTITY)
					.then(idArgWithSuggestions(DetectionKind.ENTITY)
						.then(ClientCommandManager.argument("value", intArg(0, 72000))
							.executes(ctx -> {
								...
								ctx.getSource().sendFeedback(text("detector: messageCooldown updated for " + target.id() + " -> " + value));
								DetectorMod.LOGGER.info("Command detect messageCooldown update kind={}, id={}, value={}", target.kind(), target.id(), value);
								return 1;
							}))))
				.then(kindLiteral(DetectionKind.BLOCK)
					.then(idArgWithSuggestions(DetectionKind.BLOCK)
			...
			root.then(ClientCommandManager.literal("highlightCooldown")
				.then(kindLiteral(DetectionKind.ENTITY)
					.then(idArgWithSuggestions(DetectionKind.ENTITY)
						.then(ClientCommandManager.argument("value", intArg(0, 72000))
							.executes(ctx -> {
								...
								ctx.getSource().sendFeedback(text("detector: highlightCooldown updated for " + target.id() + " -> " + value));
								DetectorMod.LOGGER.info("Command detect highlightCooldown update kind={}, id={}, value={}", target.kind(), target.id(), value);
								return 1;
							}))))
				.then(kindLiteral(DetectionKind.BLOCK)
					.then(idArgWithSuggestions(DetectionKind.BLOCK)
	...
	private static DetectionTarget requireTarget(
		...
		if (target == null) {
			ctx.getSource().sendError(text("Target not configured. Add it first, e.g. /detector "
				+ kind.name().toLowerCase() + " " + id + " true"));
			DetectorMod.LOGGER.warn("Cannot tune unknown target kind={}, id={}", kind, id);
			return null;
		}
		return target;
	}
	...
	private static int applyPreset(
		...
		configStore.save(registry, verboseLoggingSupplier.getAsBoolean(), highlightOnMatchRef.get());
		source.sendFeedback(text(
			"detector: preset " + presetName + " applied; value=" + enabled + " changed=" + changed + " added=" + added
		));
		DetectorMod.LOGGER.info(
	...
	private static RequiredArgumentBuilder<FabricClientCommandSource, Identifier> idArgWithSuggestions(DetectionKind kind) {
		...
		if (kind == DetectionKind.ENTITY) {
			return builder.suggests(DetectorClientCommands::suggestEntityIds);
		}
		return builder.suggests(DetectorClientCommands::suggestBlockIds);
	}
